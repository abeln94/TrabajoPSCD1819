...
LindaDriver::LindaDriver(string ip, int puerto) {
  ...
  // Buffer content:
  // 1:000.000.000.000,XXXX|2:000.000.000.000,XXXX|3:000.000.000.000,XXXX
  string buffer;
  int length = 100;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    rellenar(buffer[0] - '0', buffer.substr(2, found1 - 2),
             atoi(buffer.substr(found1 + 1, found2 - 1).c_str()));
...
void LindaDriver::test_server(int numTuplas) {
  switch (numTuplas) {
    ...
    case 6:
      ...
      break;
    default:
      cout << "[Linda] Error en test_server: numTuplas > 6 || numTuplas < 1"
           << endl;
  }
}
...
void LindaDriver::PN(Tupla mensaje) {
  ...
  if (rcv_bytes == -1) {
    string mensError = strerror(errno);
    cerr << "[Linda][PN] Error al recibir mensaje del subservidor: " +
                mensError + "\n";
    exit(0);
  }
  if (buffer != "OK") {
...
Tupla LindaDriver::RN(Tupla mensaje) {
  ...
  //			 y el tamaño de la tupla para que pueda tratar el
  // mensaje.
  ...
  if (rcv_bytes == -1) {
    string mensError = strerror(errno);
    cerr << "[Linda][RN] Error al recibir mensaje del subservidor: " +
                mensError + "\n";
    exit(0);
  }
...
Tupla LindaDriver::readN(Tupla mensaje) {
  ...
  if (snd_bytes == -1) {
    string mensError = strerror(errno);
    cerr << "[Linda][readN] Error al enviar mensaje al subservidor: " +
                mensError + "\n";
    exit(0);
  }
  string buffer;
  ...
  if (rcv_bytes == -1) {
    string mensError = strerror(errno);
    cerr << "[Linda][readN] Error al recibir mensaje del subservidor: " +
                mensError + "\n";
    exit(0);
  }
  Tupla mens_final(mensaje.size());
  ...
  if (crear) {
    ...
    cout << "[Linda][readN] Error en la tupla enviado por el subservidor"
         << endl;
  }
}
