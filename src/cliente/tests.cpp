...
void clienteCarga(char* ip, int port, int _, char* command) {
    ...
    //    return 1;
    //}
    int NT = atoi(/*argv[1]*/command); //num de tuplas a tratar
    // un driver con los datos pasados en la invocación
    LindaDriver LD(/*argv[2], argv[3], ...*/ip,port);
    // La conexión con el servidor Linda ya está establecido
    //-----------------------------------------------------
    ...
    for (int i=1; i<=NT; i++) {
        ...
        for (int j=0; j<MAX_LENGTH; j++) {
            tuplas[j]->from_string('[' + vT[j] + ']');//fixed
            // PosNote
            LD.PN(*tuplas[j]);//fixed
        }
    }
    ...
    for (int i=1; i<=NT; i++) {
        for (int j=0; j<MAX_LENGTH; j++) {
            // RemoveNote
            *tuplas[j] = LD.RN(*patrones[j]); //misma longitud //fixed
        }
        const int GRUPO = 50;
        // mostrar primera y última tuplas de cada grupo de GRUPO tuplas rescatadas
        if ((i-1)%GRUPO == 0) {
            cout << "    " << tuplas[0]->to_string() << endl;//fixed
            cout << "    " << tuplas[MAX_LENGTH-1]->to_string() << endl;//fixed
        }
    }
...
ADDFUNCTION( clienteCarga, "cliente de ejemplo profesores. Ejecutar como '1 clienteCarga N' siendo N el número de tuplas");