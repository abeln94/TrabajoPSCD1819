...
#define ADDFUNCTION(name, description)                                   \
  \
class _Class##name{public : _Class##name() {cout << "<" << #name << "> " \
                                                 << description << endl; \
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 6 || argc % 3 != 0) {
    ...
    cout << "       nX nameX paramX means: run xN threads as function nameX "
            "with parameter paramX (all of them)" << endl;
    return 0;
  }
  ...
  for (int i = 0; i < Nfunctions; ++i) {
    ...
    cout << "Running " << instances << " thread" << (instances == 1 ? "" : "s")
         << " as function '" << name << "' with parameter '" << param << "'"
         << endl;
