...
void t_cond(char* ip, int port, int _, char* param){
  ...
  for(int i = 0; i < max; ++i){
	...
	graf << tamano[1] << "," << querer[1] << "," << querer[2] << "," << querer[3] << endl;
	int n = stoi(tamano[1]);
...
ADDFUNCTION( t_medio, "Gráfico, ejecutar como 'M t_medio .' siendo M el número de clientes");
...
ADDFUNCTION( t_conc, "Gráfico, ejecutar como '1 t_conc N' siendo N el número de veces y el número de tuplas siguiendo este formato: 'veces:tuplas'");
